...
class BaseTuner(nn.Module, ABC):
    ...
    def __init__(self, model, peft_config: Union[PeftConfig, dict[str, PeftConfig]], adapter_name: str) -> None:
        # print(__file__, "ON NO AGAIN! BaseTuner")
        super().__init__()
        ...
        # print(__file__, "trying to inject adapter in BaseTuner...")
    ...
    def inject_adapter(self, model: nn.Module, adapter_name: str):
        ...
        print(__file__, "prepare peft_config:", peft_config)
        ...
        for key in key_list:
            ...
            # use modified _get_submodules to get correct path
            
            def _modified_get_submodules(model, key):
                # choose the shortest existing path in taget_modules
                nodes = key.split(".")
                for i in range(1, len(nodes)):
                    target_name = (".".join(nodes[-i:]))
                    # print(">>>", target_name)
                    if target_name in peft_config.target_modules:
                        parent = model.get_submodule(".".join(nodes[:-i]))
            ...
            parent, target, target_name = _modified_get_submodules(model, key)
